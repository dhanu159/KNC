        ...
        function AddToGrid() {
            if ($("input[name=cmbItem]").val(0), $("input[name=txtUnitPrice]").val(), $("input[name=txtQty]").val() == "") {
                ...
                if ($("#cmbItem option:selected").val() > 0) {
                    ...
                    $(".first-tr").after('<tr><td class="itemID" name="itemID[]" id="itemID_' + row_id + '" hidden>' + itemID + '</td><td class="itemName" name="itemName[]" id="itemName_' + row_id + '">' + item + '</td><td style="text-align:right;" name="unitPrice[]" id="unitPrice_' + row_id + '">' + parseFloat(unitPrice).toFixed(2) + '</td><td style="text-align:right;" name="qty[]" id="qty_' + row_id+'">' + qty + '</td><td class="total" style="text-align:right;">' + parseFloat(total).toFixed(2) + '</td><td class="static"><span class="button red center-items"><i class="fas fa-times"></i></span></td></tr>');
                    row_id++;
                    remove();
